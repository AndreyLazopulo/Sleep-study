function [plambda AIC gof pval LL] = Gaus_3Expo_mle(distributdata,plotting,binway)
...
while isempty(startpoint) && c<=2
    Allstartvals2=loghistfit5(distributdata,0.2,4,300,0,0,1);
    Allstartvals1=loghistfit5(distributdata,0.2,4,300,0,0,2);
...
parfor gg=1:min([70,size(startpoint,1)])
    ...
    try
        oparm(gg,:)=fmincon(@(A) binned_gaus_3expo_pdf(distributdata,bin_bound,A),startpoint(gg,:),[],[],[],[],lowerbound,upperbound,[],specoption);
        totLL(gg,1)=-binned_gaus_3expo_pdf(distributdata,bin_bound,oparm(gg,:));
    catch
        continue
    end
...
[Obs Expect] = GetObsExpect_AL(distributdata,'Gaus3Exponential',plambda,1);
...
%     end
    function PrVals=Prfunc(A,lb,ub)
        ...
        a1=A(1); a2=A(2); a3=A(3); mu=A(4); sig=A(5); lam2=exp(A(6)); lam3=exp(A(7)); lam4=exp(A(8));
        %calculate expo term
        %without xmax
...
%         expo_term4=(1-a1-a2-a3)*exp(lam4*xmin)*(exp(-lam4*lb)-exp(-lam4*ub));
        ...
        expo_term3 = reshape(expo_term3, 1, numel(expo_term3));
        expo_term4 = reshape(expo_term4, 1, numel(expo_term4));
        PrVals=((expo_term4)+(expo_term3)+(expo_term2)+(gaussian_term));
    end
