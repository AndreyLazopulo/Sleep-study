function [plambda AIC gof pval LL] = Gamma_1Expo_mle(distributdata,plotting,binway)
...
while isempty(startpoint) && c<=2
    Allstartvals2=loghistfit6(distributdata,0.2,2,300,0,0,1);
    Allstartvals1=loghistfit6(distributdata,0.2,2,300,0,0,2);
...
lowerbound=[miny -10 log(minlambda) log(minlambda)];
upperbound=[maxy 10 log(maxlambda) log(maxlambda)];
oparm=NaN(70,2*k); totLL=NaN(70,1);

parfor gg=1:min([70,size(startpoint,1)])
    
    try
        oparm(gg,:)=fmincon(@(A) binned_gamma_1expo_pdf(distributdata,bin_bound,A),startpoint(gg,:),[],[],[],[],lowerbound,upperbound,[],specoption);
        totLL(gg,1)=-binned_gamma_1expo_pdf(distributdata,bin_bound,oparm(gg,:));
    catch
        continue
    end
...
[Obs Expect] = GetObsExpect_AL(distributdata,'Gamma1Exponential',plambda,1);
...
%     end
    function PrVals=Prfunc(A,lb,ub)
        ...
        a1=A(1); alpha=A(2); lam1=exp(A(3)); lam2=exp(A(4));
        %calculate expo term
        %without xmax
