function [plambda AIC gof pval LL] = Gaus_2Expo_mle(distributdata,plotting,binway)
...
while isempty(startpoint) && c<=2
    Allstartvals2=loghistfit5(distributdata,0.2,3,300,0,0,1);
    Allstartvals1=loghistfit5(distributdata,0.2,3,300,0,0,2);
...
lowerbound=[miny miny min(distributdata) 1 log(minlambda) log(minlambda)];
upperbound=[maxy maxy max(distributdata) sqrt(var(distributdata)) log(maxlambda) log(maxlambda)];
oparm=NaN(70,2*k); totLL=NaN(70,1);

parfor gg=1:min([70,size(startpoint,1)])
    
    try
        oparm(gg,:)=fmincon(@(A) binned_gaus_2expo_pdf(distributdata,bin_bound,A),startpoint(gg,:),[],[],[],[],lowerbound,upperbound,[],specoption);
        totLL(gg,1)=-binned_gaus_2expo_pdf(distributdata,bin_bound,oparm(gg,:));
    catch
        continue
    end
...
[Obs Expect] = GetObsExpect_AL(distributdata,'Gaus2Exponential',plambda,1);
...
%         expo_term4=(1-a1-a2-a3)*exp(lam4*xmin)*(exp(-lam4*lb)-exp(-lam4*ub));
        ...
        expo_term2 = reshape(expo_term2, 1, numel(expo_term2));
        expo_term3 = reshape(expo_term3, 1, numel(expo_term3));
        PrVals=((expo_term3)+(expo_term2)+(gaussian_term));
    end
