function [plambda AIC gof pval LL] = Binned_4Expo_mle(distributdata,plotting,binway)
...
parfor gg=1:min([50,size(startpoint,1)])
    ...
    try
        ...
        totLL(gg,1)=-binned_4expo_pdf(distributdata,bin_bound,oparm(gg,:));
    catch
        continue
    end
...
[Obs Expect] = GetObsExpect_AL(distributdata,'4Exponential',plambda,1);
...
%     end
    function PrVals=Prfunc(A,lb,ub)
        ...
        expo_term3 = reshape(expo_term3, 1, numel(expo_term3));
        expo_term4 = reshape(expo_term4, 1, numel(expo_term4));
        PrVals=((expo_term4)+(expo_term3)+(expo_term2)+(expo_term1));
    end
