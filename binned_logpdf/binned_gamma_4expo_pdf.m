function fvals=binned_gamma_4expo_pdf(x,bin_bound,A)
...
%         expo_term4=(1-a1-a2-a3)*exp(lam4*xmin)*(exp(-lam4*l)-exp(-lam4*u));
        ...
        expo_term5=(1-a1-a2-a3-a4)*(exp(-lam5*l)-exp(-lam5*u))./(exp(-lam5*xmin)-exp(-lam5*xmax));
        ...
        Pr=log((expo_term5)+(expo_term4)+(expo_term3)+(expo_term2)+(gamma_term));
        fvals=-sum(h.*Pr);
    end