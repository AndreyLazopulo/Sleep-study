function fvals=binned_pl4expo_pdf(x,bin_bound,A)
...
%         expo_term4=(1-a1-a2-a3)*(gamma_incomplete(lam4*l,1-alpha)-gamma_incomplete(lam4*u,1-alpha))./(gamma_incomplete(lam4*xmin,1-alpha));
        %with xmax
        ...
        expo_term4=(1-a1-a2-a3)*(gamma_incomplete(lam4*l,1-alpha)-gamma_incomplete(lam4*u,1-alpha))./(gamma_incomplete(lam4*xmin,1-alpha)-gamma_incomplete(lam4*xmax,1-alpha));
        ...
        Pr=log((expo_term1)+(expo_term2)+(expo_term3)+(expo_term4));
        fvals=-sum(h.*Pr);
    end
