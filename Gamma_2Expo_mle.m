function [plambda AIC gof pval LL] = Gamma_2Expo_mle(distributdata,plotting,binway)
...
while isempty(startpoint) && c<=2
    Allstartvals2=loghistfit6(distributdata,0.2,3,300,0,0,1);
    Allstartvals1=loghistfit6(distributdata,0.2,3,300,0,0,2);
...
lowerbound=[miny miny -10 log(minlambda) log(minlambda) log(minlambda)];
upperbound=[maxy maxy 10 log(maxlambda) log(maxlambda) log(maxlambda)];
oparm=NaN(70,2*k); totLL=NaN(70,1);

parfor gg=1:min([70,size(startpoint,1)])
    
    try
        oparm(gg,:)=fmincon(@(A) binned_gamma_2expo_pdf(distributdata,bin_bound,A),startpoint(gg,:),[],[],[],[],lowerbound,upperbound,[],specoption);
        totLL(gg,1)=-binned_gamma_2expo_pdf(distributdata,bin_bound,oparm(gg,:));
    catch
        continue
    end
...
[Obs Expect] = GetObsExpect_AL(distributdata,'Gamma2Exponential',plambda,1);
...
%     end
    function PrVals=Prfunc(A,lb,ub)
        ...
        a1=A(1); a2=A(2); alpha=A(3); lam1=exp(A(4)); lam2=exp(A(5)); lam3=exp(A(6));
        %calculate expo term
        %without xmax
...
%         expo_term4=(1-a1-a2-a3)*exp(lam4*xmin)*(exp(-lam4*lb)-exp(-lam4*ub));
        ...
        expo_term2 = reshape(expo_term2, 1, numel(expo_term2));
        expo_term3 = reshape(expo_term3, 1, numel(expo_term3));
        PrVals=((expo_term3)+(expo_term2)+(gamma_term));
    end
