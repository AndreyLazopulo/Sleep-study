function [Obs Expect] = GetObsExpect_AL2(data,mycdf,params,min_bin)
...
trimmededges1=x(1:end-1);
trimmededges2=x(2:end);

% P = num2cell(params) cdfvalues=feval(mycdf,trimmededges,P{:});
% cdfvalues=[0;cdfvalues(:);1];

[xx xxx cdfvalues1] = CalcCDF(trimmededges1,mycdf,params,min(data),max(data));
[xx xxx cdfvalues2] = CalcCDF(trimmededges2,mycdf,params,min(data),max(data));
% cdfvalues=[0;cdfvalues(:);1];
if max(cdfvalues2)<0.9999
    cdfvalues2=[cdfvalues2(:);0.9999];
end
if size(cdfvalues2,2)==1
    cdfvalues2=cdfvalues2';
end
if size(cdfvalues1,2)==1
    cdfvalues1=cdfvalues1';
end
cdfvalues=[cdfvalues1;cdfvalues2];
