function [plambda AIC gof pval LL] = Gamma_4Expo_mle(distributdata,plotting,binway)
...
while isempty(startpoint) && c<=2
    Allstartvals2=loghistfit6(distributdata,0.2,5,300,0,0,1);
    Allstartvals1=loghistfit6(distributdata,0.2,5,300,0,0,2);
...
lowerbound=[miny miny miny miny -10 log(minlambda) log(minlambda) log(minlambda) log(minlambda) log(minlambda)];
upperbound=[maxy maxy maxy maxy 10 log(maxlambda) log(maxlambda) log(maxlambda) log(maxlambda) log(maxlambda)];
oparm=NaN(70,2*k); totLL=NaN(70,1);

parfor gg=1:min([70,size(startpoint,1)])
    
    try
        oparm(gg,:)=fmincon(@(A) binned_gamma_4expo_pdf(distributdata,bin_bound,A),startpoint(gg,:),[],[],[],[],lowerbound,upperbound,[],specoption);
        totLL(gg,1)=-binned_gamma_4expo_pdf(distributdata,bin_bound,oparm(gg,:));
    catch
        continue
    end
...
[Obs Expect] = GetObsExpect_AL(distributdata,'Gamma4Exponential',plambda,1);
[gof pval] = Gtest(Obs,Expect,n_params); %do the G-test

if plotting == 1
    fprintf('a-parameters : %5.4f \t %5.4f \t %5.4f \t %5.4f \t %5.4f \n',plambda(1:k))
    fprintf('alpha and lambda parameters : %5.4f \t %5.4f \n',plambda(k+1:k+2))
...
%     end
    function PrVals=Prfunc(A,lb,ub)
        ...
        a1=A(1); a2=A(2); a3=A(3); a4=A(4); alpha=A(5); lam1=exp(A(6)); lam2=exp(A(7)); lam3=exp(A(8)); lam4=exp(A(9)); lam5=exp(A(10));
        %calculate expo term
        %without xmax
...
%         expo_term4=(1-a1-a2-a3)*exp(lam4*xmin)*(exp(-lam4*lb)-exp(-lam4*ub));
        %with xmax
        gamma_term=a1*(gamma_incomplete(lam1*lb,1-alpha)-gamma_incomplete(lam1*ub,1-alpha))./(gamma_incomplete(lam1*xmin,1-alpha)-gamma_incomplete(lam1*xmax,1-alpha)); 
        ...
        PrVals=((expo_term5)+(expo_term4)+(expo_term3)+(expo_term2)+(gamma_term));
    end
